...
def test_ansible_core_is_installed_at_version_2_12_3_(host):
    cmd = host.run("pip3 show --disable-pip-version-check ansible-core")
    assert cmd.rc is 0
    assert "Name: ansible-core\nVersion: 2.12.3" in cmd.stdout
...
def test_ansible_version_command_reports_core_version_2_12_3_(host):
    assert 'core 2.12.3' in host.run('ansible --version').stdout
